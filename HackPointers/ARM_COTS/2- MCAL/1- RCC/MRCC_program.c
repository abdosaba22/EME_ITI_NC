...
STD_error_t MRCC_stderrCLKConfig( MRCC_Config_t* ARG_pconfCLKConfig)
{
	...
	/* set AHB Prescaler  */
	MRCC->CFGR &= ~(RCC_CFGR_AHBPRE_MASK);   /* ~(0b1111<<4) */
	MRCC->CFGR |= (ARG_pconfCLKConfig->AHBPRE_Value << RCC_CFGR_HPRE_0);   /* (CLK_SOURCE<<0) */
	u8 L_u8Counter = 0;
	while( L_u8Counter++ < 17 ); /* The clocks are divided with the new prescaler factor from 1 to 16 AHB cycles after HPRE write. */
	...
	if(ARG_pconfCLKConfig != NULL_POINTER )
	{
		...
		L_stderrState = !(((MRCC->CFGR & RCC_CFGR_SWS_MASK) >> 2) == ARG_pconfCLKConfig->CLK_Type );
